...
__global__ void convolution(const float *f, const float *g, float *o, const int width, const int height, const int radius_x, const int radius_y) {
...
int main() {
    ...
    cudaMemcpy(d_g, g, kernel_width * kernel_height * sizeof(float), cudaMemcpyHostToDevice);
    ...
    convolution<<<blocksPerGrid, threadsPerBlock>>>(d_f, d_g, d_o, n_x, n_y, radius_x, radius_y);
